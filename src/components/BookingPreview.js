...
import { reset } from '../redux/bookingForm/actions'

const BookingPreview = ({Bookings}) => {
...
const dispatch = useDispatch()
    ...
    if(Bookings.length === 0){
        return <div className="text-center text-2xl text-gray-500">No Bookings</div>
    }
      ...
      <tbody class="divide-y divide-gray-300/20" id="lws-previewBooked">
     
        {
            Bookings.map((booking) => (
                 <tr key={booking.id} class="lws-bookedTable text-black">
                 <td class="px-6 py-4">
                 ...
                 <td class="px-6 py-4 text-center">
                   <p class="lws-bookedDate">{booking?.dateFrom}</p>
                 </td>
                 <td class="px-6 py-4 text-center">
                   <p class="lws-bookedGustes">{booking?.guest}</p>
                 </td>
                 <td class="px-6 py-4 text-center">
                   <span class="lws-bookedClass"> {booking?.flightTypeClass} </span>
                 </td>
                 <td class="px-6 py-4 text-center">
                   <div class="flex justify-center gap-4">
                     <button onClick={()=>dispatch(reset(booking?.id))} class="lws-remove">
